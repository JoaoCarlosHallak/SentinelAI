...
public class OpenRedirectScanServiceImpl implements OpenRedirectScanService {
    ...
    @Autowired
    public OpenRedirectScanServiceImpl(HttpClientService httpClientService, LoadPayloadsService loadPayloadsService, ClientOllamaService clientOllamaService) {
    ...
    public Flux<HttpResponseDataDTO> scanAndBasicFilter(String target) throws Exception {
        ...
        List<String> payloads = ScanHelperUtils.mixPayloadAndTarget(loadPayloadsService.loadPayloads("payloads/op.txt"), target);
        ...
        return Flux.fromIterable(payloads)
            ...
            .filter(res -> {
                String location = res.header().getFirst("Location");
                return location != null && location.contains(res.payload());
})
            .map(res -> {
            ...
            .doOnNext(res ->
                System.out.println("Open Redirect possible: " + res.url() +
            ...
            .flatMap(results -> {
                ...
                return Mono.fromCallable(() ->
                    clientOllamaService.sendRequest(
                        SystemPrompts.buildOpenRedirectAnalysisPrompt(results.get(0))
                    )
