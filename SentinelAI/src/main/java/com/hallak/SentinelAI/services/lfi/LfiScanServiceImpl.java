...
public class LfiScanServiceImpl implements LfiScanService {
    ...
    @Autowired
    public LfiScanServiceImpl(HttpClientService httpClientService, LoadPayloadsService loadPayloadsService, ClientOllamaService clientOllamaService) {
    ...
    public Flux<HttpResponseDataDTO> scanAndBasicFilter(String target) throws Exception { 
        ...
        List<String> payloads = ScanHelperUtils.mixPayloadAndTarget(loadPayloadsService.loadPayloads("payloads/lfi.txt"), target);
        List<String> lfiContents = loadPayloadsService.loadPayloads("payloads/lfiContent.txt");
    ...
    @Override
    public Mono<String> handleLfiScanner(String target) throws Exception { // Retorna string no futuro
            ...
            .flatMap(results -> { // Quando a lista chegar exercute isso, mas nao trave a thread
                ...
                return Mono.fromCallable(() ->
                    clientOllamaService.sendRequest(
                        SystemPrompts.buildLFIAnalysisPrompt(results.get(0))
                    )
